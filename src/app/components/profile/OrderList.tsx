...
export default function OrderList() {
  ...
  useEffect(() => {
    const fetchOrdersAndReviews = async () => {
      try {
        ...
        setReviewStatus(reviewsRes.data.data);
      } catch (err) {
  ...
  const canReview = (order: Order, item: OrderItem) => {
    if (order.status !== "paid") return false;
    const review = reviewStatus.find(
      (r) =>
        r.product_id === item.product.id && r.variant_id === item.variant_id
    );
