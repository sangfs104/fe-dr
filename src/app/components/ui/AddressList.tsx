...
export default function AddressList() {
  ...
  return (
    <section className="bg-white p-6 sm:p-8 rounded-xl shadow-sm space-y-6">
      <h2 className="text-xl font-semibold text-orange-600">
        Danh sách địa chỉ nhận hàng
      </h2>
      ...
      >
        <input
          type="text"
          className="flex-1 border rounded-lg px-4 py-3 text-sm focus:ring-2 focus:ring-orange-500 focus:outline-none w-full"
          value={newAddress}
          onChange={(e) => setNewAddress(e.target.value)}
          placeholder="Nhập địa chỉ mới..."
          required
        ...
        <button
          type="submit"
          className="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-600 w-full sm:w-auto"
          disabled={submitting}
        >
          {submitting ? "Đang thêm..." : "Thêm"}
        </button>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {addresses.map((addr) => (
            <div
              key={addr.id}
              className={`border rounded-lg p-4 text-sm relative break-words ${
                addr.is_default === 1
                  ? "border-orange-500 bg-orange-50"
                  : "hover:border-orange-300"
              }`}
