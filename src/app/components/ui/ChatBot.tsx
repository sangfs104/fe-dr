...
export default function ChatBox({ onClose }: { onClose: () => void }) {
  ...
  //     const { message, products, mix_and_match, keywords } = res.data;
  //     let reply =
  //       message ||
  //       "Chào bạn! Mình chưa hiểu rõ gu của bạn lắm. Bạn có thể mô tả thêm một chút không ạ?";
  //     let productList = products || [];
  ...
  //     if (products && products.length > 0) {
  //       reply = `Chào bạn! Mình đã tìm thấy một sản phẩm rất phù hợp cho bạn là ${products[0].name}. Bạn thấy thế nào ạ?`;
  //     } else if (mix_and_match) {
  //       reply = `Mình có gợi ý phối đồ cho bạn đây: ${mix_and_match}. Bạn có thích không ạ?`;
  //     }
  ...
  //         type: "bot",
  //         text: reply,
  //         products: Array.isArray(productList) ? productList : [productList],
  //         keywords: keywords || [],
  //         mix_and_match: mix_and_match || null,
  //         timestamp: new Date(),
  //       },
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Kiểm tra nếu productList chứa ít nhất một sản phẩm hợp lệ
      if (productList.length > 0 && productList[0].length > 0) {
        reply = `Chào bạn! Mình đã tìm thấy một sản phẩm rất phù hợp cho bạn là ${productList[0][0].name}. Bạn thấy thế nào ạ?`;
      } else if (mix_and_match) {
        reply = `Mình có gợi ý phối đồ cho bạn đây: ${mix_and_match}. Bạn có thích không ạ?`;
      }
      ...
      setMessages((prev) => [
        ...prev,
        {
          type: "bot",
          text: reply,
          products: Array.isArray(productList[0]) ? productList[0] : [],
          keywords: keywords || [],
          mix_and_match: mix_and_match || null,
          timestamp: new Date(),
        },
