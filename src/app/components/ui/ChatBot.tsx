...
//         {/* Enhanced chat container with glassmorphism effect */}
//         <div className="rounded-3xl shadow-2xl bg-gradient-to-b from-slate-900/95 to-slate-800/95 backdrop-blur-xl text-white border border-slate-700/50 overflow-hidden transition-all duration-500 ease-in-out">
...
//               <div className="flex items-center gap-3">
//                 <div className="relative">
//                   <div className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white/20 backdrop-blur-sm border border-white/30">
...
//                   className="inline-flex items-center justify-center h-9 w-9 rounded-xl bg-white/20 hover:bg-white/30 transition-all duration-300 backdrop-blur-sm border border-white/30"
//                 >
...
//                 <button
//                   onClick={handleClose}
//                   className="inline-flex items-center justify-center h-9 w-9 rounded-xl bg-red-500/80 hover:bg-red-500 transition-all duration-300 backdrop-blur-sm"
//                 >
//                   <X className="h-4 w-4" />
//                 </button>
//               </div>
...
//             {/* Settings panel */}
//             {showSettings && (
//               <div className="absolute top-full left-0 right-0 mt-2 p-4 bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-xl z-10">
//                 <div className="space-y-4">
//                   <div className="flex items-center justify-between">
//                     <span className="text-sm font-medium">Đọc tin nhắn</span>
//                     <button
...
//                       <div>
//                         <label className="text-sm font-medium block mb-2">
//                           âm lượng: {Math.round(speechVolume * 100)}%
//                         </label>
//                         <input
//                           type="range"
//                           min="0"
...
//               {/* Enhanced input area */}
//               <div className="bg-gradient-to-t from-slate-900/50 to-transparent p-4">
//                 <div className="relative">
//                   <textarea
//                     ref={textareaRef}
//                     value={input}
//                     onChange={(e) => setInput(e.target.value)}
//                     onKeyDown={onKeyDown}
//                     placeholder="Nhập hoặc nói để hỏi: tìm sản phẩm, hỏi size, xem giảm giá/flash sale..."
//                     className="w-full resize-none bg-slate-800/80 backdrop-blur-sm text-white rounded-2xl p-4 pr-24 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-purple-500/50 border border-slate-700/50 transition-all duration-300"
//                     rows={1}
//                   />
...
//                       disabled={sending || !SpeechRecognition}
//                       className={clsx(
//                         "inline-flex items-center gap-1 rounded-xl px-3 py-2 text-sm font-medium transition-all duration-300",
//                         sending || !SpeechRecognition
...
//                       disabled={sending || !input.trim()}
//                       className={clsx(
//                         "inline-flex items-center gap-1 rounded-xl px-4 py-2 text-sm font-medium transition-all duration-300",
//                         sending || !input.trim()
//                           ? "bg-slate-700 text-slate-400 cursor-not-allowed"
...
//     if (!speechEnabled || !msg.text || msg.role !== "assistant") return;
...
//     utteranceRef.current = utterance;
//     window.speechSynthesis.speak(utterance);
//   }, [msg.text, msg.role, speechEnabled, speechRate, speechVolume]);
...
//   useEffect(() => {
//     if (msg.role === "assistant" && msg.text && speechEnabled) {
//       // Auto-speak new assistant messages
//       const timer = setTimeout(speak, 500);
//       return () => clearTimeout(timer);
//     }
//   }, [msg.text, msg.role, speechEnabled, speak]);
...
//       <div
//         className={clsx(
//           "max-w-[85%] rounded-3xl px-4 py-3 text-sm relative transition-all duration-300 hover:shadow-lg",
//           isUser
...
//         {!isUser && speechEnabled && (
//           <button
//             onClick={isSpeaking ? stopSpeaking : speak}
//             className="absolute -top-2 -right-2 inline-flex items-center justify-center h-6 w-6 bg-slate-700 hover:bg-slate-600 rounded-full text-xs transition-all duration-200"
//           >
//             {isSpeaking ? (
...
//   if (!name && !desc && (!keywords || keywords.length === 0)) return null;
//   return (
//     <div className="rounded-2xl border border-slate-600/50 bg-slate-800/60 backdrop-blur-sm p-4 transition-all duration-300 hover:bg-slate-700/60">
...
//             <span
//               key={i}
//               className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 px-3 py-1 text-xs text-purple-200 backdrop-blur-sm transition-all duration-300 hover:scale-105"
//             >
//               <Tag className="h-3 w-3" /> {k}
//             </span>
//           ))}
...
//   return (
//     <div className="rounded-2xl overflow-hidden border border-slate-600/50 bg-slate-800/40 backdrop-blur-sm transition-all duration-300 hover:bg-slate-700/40 hover:scale-105 hover:shadow-xl">
...
//         {hasVariants && variants.length > 0 && (
//           <div className="overflow-x-auto bg-slate-700/30 rounded-xl p-3 mt-3">
//             <table className="min-w-full text-xs">
//               <thead>
...
//                   <th className="text-left font-medium pr-3 py-2">Giá</th>
...
//                   <tr
//                     key={v.id ?? i}
//                     className="text-slate-300 hover:bg-slate-600/30 transition-colors"
//                   >
//                     <td className="pr-3 py-2">
//                       <span className="inline-block bg-slate-600 px-2 py-1 rounded text-xs">
//                         {v.size ?? "-"}
//                       </span>
//                     </td>
//                     <td className="pr-3 py-2">
  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 right-4 z-50 w-full max-w-md">
        ...
        {/* Chat container without blur */}
        <div className="rounded-3xl shadow-2xl bg-gradient-to-b from-gray-900 to-gray-800 text-white border border-gray-700 overflow-hidden transition-all duration-500 ease-in-out">
          {/* Header with Shopee orange */}
          <div className="relative bg-gradient-to-r from-orange-600 to-orange-500 p-4">
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-white/20 border border-white/30">
                    <Sparkles className="h-5 w-5 text-white" />
                  </div>
                  <div className="absolute -top-1 -right-1 h-4 w-4 bg-green-400 rounded-full border-2 border-white animate-pulse"></div>
                </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setShowSettings(!showSettings)}
                  className="inline-flex items-center justify-center h-9 w-9 rounded-xl bg-white/20 hover:bg-white/30 transition-all duration-300 border border-white/30"
                >
                  <Settings className="h-4 w-4" />
                </button>
                <button
                ...
                <button
                  onClick={handleClose}
                  className="inline-flex items-center justify-center h-9 w-9 rounded-xl bg-red-500/80 hover:bg-red-500 transition-all duration-300"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
            ...
            {/* Settings panel without blur */}
            {showSettings && (
              <div className="absolute top-full left-0 right-0 mt-2 p-4 bg-gray-900 rounded-2xl border border-gray-700 shadow-xl z-10">
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Đọc tin nhắn</span>
                    <button
          ...
          {!isMinimized && (
            <>
              <div
                ref={listRef}
                className="max-h-[65vh] overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent hide-scrollbar"
              >
                {messages.map((m) => (
                  <MessageBubble
                    key={m.id}
              ...
              <div className="bg-gradient-to-t from-gray-900 to-transparent p-4">
                <div className="relative">
                  <textarea
                    ...
                    onKeyDown={onKeyDown}
                    placeholder="Nhập hoặc nói để hỏi: tìm sản phẩm, hỏi size, xem giảm giá/flash sale..."
                    className="w-full resize-none bg-gray-800 text-white rounded-2xl p-4 pr-24 placeholder:text-gray-400 outline-none focus:ring-2 focus:ring-orange-500/50 border border-gray-700 transition-all duration-300"
                    rows={1}
                  />
                  ...
                  <div className="absolute right-2 bottom-2 flex gap-2">
                    ...
                    <button
                      ...
                      className={clsx(
                        ...
                        sending || !input.trim()
                          ? "bg-gray-700 text-gray-400 cursor-not-allowed"
                          : "bg-gradient-to-r from-orange-600 to-orange-500 text-white hover:from-orange-700 hover:to-orange-600 hover:scale-105 shadow-lg"
                      )}
                    >
                      {sending ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
      ...
      <style jsx>{`
        .slider::-webkit-slider-thumb {
          ...
          width: 16px;
          border-radius: 50%;
          background: linear-gradient(45deg, #ff5722, #ee4d2d);
          cursor: pointer;
          border: 2px solid #ffffff;
        }
    ...
    <div className={clsx("flex", isUser ? "justify-end" : "justify-start")}>
      ...
      >
        ...
        {!isUser && speechEnabled && (
          <button
            onClick={isSpeaking ? stopSpeaking : speak}
            className="absolute -top-2 -right-2 inline-flex items-center justify-center h-6 w-6 bg-gray-700 hover:bg-gray-600 rounded-full text-xs transition-all duration-200"
          >
            {isSpeaking ? (
              <Pause className="h-3 w-3" />
            ) : (
...
function StyleSummary({
  ...
  if (!name && !desc && (!keywords || keywords.length === 0)) return null;
  return (
    <div className="rounded-2xl border border-gray-600 bg-gray-800 p-4 transition-all duration-300 hover:bg-gray-700">
      ...
      {keywords && keywords.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {keywords.map((k, i) => (
            <span
              key={i}
              className="inline-flex items-center gap-1 rounded-full bg-orange-500/20 border border-orange-500/30 px-3 py-1 text-xs text-orange-200 transition-all duration-300 hover:scale-105"
            >
              <Tag className="h-3 w-3" /> {k}
            </span>
          ))}
...
function ProductCard({
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-600 bg-gray-800 transition-all duration-300 hover:bg-gray-700 hover:scale-105 hover:shadow-xl">
      <div className="aspect-[16/9] bg-gray-700 relative overflow-hidden">
        ...
        {displayPrice && (
          <div className="absolute top-2 right-2 bg-gradient-to-r from-orange-600 to-orange-500 text-white text-xs font-bold px-2 py-1 rounded-full">
            {formatPrice(displayPrice)}
          </div>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        {hasVariants && variants.length > 0 && (
          <div className="overflow-x-auto bg-gray-700 rounded-xl p-3 mt-3">
            <table className="min-w-full text-xs">
              <thead>
              ...
              <tbody>
                {variants.map((v, i) => (
                  <tr
                    key={v.id ?? i}
                    className="text-gray-300 hover:bg-gray-600 transition-colors"
                  >
                    <td className="pr-3 py-2">
                      <span className="inline-block bg-gray-600 px-2 py-1 rounded text-xs">
                        {v.size ?? "-"}
                      </span>
                    </td>
                    <td className="pr-3 py-2">
