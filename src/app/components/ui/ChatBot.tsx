    ...
    try {
      setTimeout(async () => {
        const res = await axios.post(
          ...
          {
            answers: [input],
            searchType: input.match(
              /(phối đồ|set đồ|đi chơi|du lịch|outfit|mix and match)/iu
            )
              ? "outfit"
              : "product",
            exactMatch: !input.match(
              /(phối đồ|set đồ|đi chơi|du lịch|outfit|mix and match)/iu
            ), // Yêu cầu khớp chính xác cho tên sản phẩm
          }
        );
        ...
        let reply =
          message ||
          "Chào bạn! Mình chưa hiểu rõ yêu cầu của bạn. Bạn có thể mô tả thêm một chút không ạ?";
        ...
        if (
          ...
          if (products && products.length > 0) {
            reply += `\nMình cũng tìm thấy một số sản phẩm phù hợp, bạn có muốn xem không ạ?`;
            productList = products.slice(0, 2); // Giới hạn 2 sản phẩm cho outfit
          }
        } else {
          // Yêu cầu tìm sản phẩm theo tên
          if (products && products.length > 0) {
            const matchedProduct = products.find(
              (p: Product) =>
                p.name.toLowerCase() === input.toLowerCase() || // Khớp chính xác
                p.name.toLowerCase().includes(input.toLowerCase()) // Khớp gần đúng
            );
            if (matchedProduct) {
              productList = [matchedProduct];
              reply = `Chào bạn! Mình đã tìm thấy sản phẩm "${matchedProduct.name}" phù hợp với yêu cầu của bạn. Bạn thấy thế nào ạ?`;
            } else {
              productList = [];
              reply = `Rất tiếc, mình không tìm thấy sản phẩm nào có tên "${input}". Bạn có muốn thử từ khóa khác hoặc xem các sản phẩm tương tự không ạ?`;
            }
          } else {
            reply = `Rất tiếc, mình không tìm thấy sản phẩm nào có tên "${input}". Bạn có muốn thử từ khóa khác không ạ?`;
          }
        }
