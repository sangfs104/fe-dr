...
const getSpinCount = (userId: string): number => {
  const key = `spin_count_${userId}`;
  return parseInt(localStorage.getItem(key) || "0", 10);
};

const increaseSpinCount = (userId: string): void => {
  const key = `spin_count_${userId}`;
...
const LuckyWheelClient = () => {
  ...
  const user = JSON.parse(localStorage.getItem("user") || "null");
  const spinCount = user?.id ? getSpinCount(user.id) : 0;
  const spinsLeft = 2 - spinCount;
    ...
    <div className="flex flex-col items-center gap-2 py-10">
      ...
      >
        <p className="text-lg text-gray-600">
          {user
            ? spinsLeft > 0
              ? `Bạn đã quay: ${spinCount}/2 lần (Còn ${spinsLeft} lượt)`
