...
import { faCartShopping, faCheck, faSpinner } from "@fortawesome/free-solid-svg-icons";
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const [mainImage, setMainImage] = useState<string | undefined>(defaultImg?.name);
          ...
          `}>
            ...
            <button
              className={`
                ...
                ${isOutOfStock ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110'}
              `}
            ...
            <button
              className={`
                action-button p-3 rounded-full shadow-lg transition-all duration-300
                ${addedToWishlist ? 'wishlist-success animate-bounce-gentle' : ''}
                hover:scale-110
              `}
          ...
          {product.img?.slice(0, 3).map((img, idx) => (
            <div
              ...
              className={`
                ...
                ${img.name === mainImage ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'}
              `}
          ...
          {selectedVariant && (
            <div className="flex items-center gap-2 text-sm">
              <div className={`w-2 h-2 rounded-full ${
                selectedVariant.stock_quantity > 5 ? 'bg-green-500' :
                selectedVariant.stock_quantity > 0 ? 'bg-orange-500' : 'bg-red-500'
              }`} />
              <span className={`text-sm ${
                selectedVariant.stock_quantity > 5 ? 'text-green-600' :
                selectedVariant.stock_quantity > 0 ? 'text-orange-600' : 'text-red-600'
              }`}>
                {selectedVariant.stock_quantity > 5 ? 'Còn nhiều' :
