...
//   price: number;
//   sale_price: string | null;
//   final_price?: number | null; // Thêm trường này
//   status: string;
// }
...
export default function ProductModal({ product, onClose }: ProductModalProps) {
  ...
  const handleAddToCart = () => {
    if (!selectedVariant || !mainImage) return;
  ...
  const handleThumbnailClick = (index: number) => {
    ...
    // Map theo `img_id` để chọn variant tương ứng
    const imageName = product.images[index].split("/").pop(); // ví dụ "sp7_1.webp"
    const imgMeta = product.img.find((img) => img.name === imageName);
    const variantForImage = product.variant.find(
      (v) => v.img_id === imgMeta?.id
    );
    if (variantForImage) setSelectedVariant(variantForImage);
  };
                        ...
                        return (
                          <motion.button
                            ...
                            onClick={() => {
                              if (!isOutOfStock) {
                                setSelectedVariant(v);
                                const imgMeta = product.img.find(
                                  (img) => img.id === v.img_id
                                );
                                if (imgMeta) {
                                  const imgFullUrl = product.images.find(
                                    (url) => url.includes(imgMeta.name)
                                  );
                                  if (imgFullUrl) setMainImage(imgFullUrl);
                                }
                              }
                            }}
                            disabled={isOutOfStock}
              ...
              <tbody>
                {product.variant.map((v) => (
                  ...
                  >
                    ...
                    <td className="py-2 px-3">{v.stock_quantity}</td>
