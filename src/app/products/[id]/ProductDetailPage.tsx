...
interface ProductDetailClientProps {
  product: Product | null; // Changed to allow null
  reviews: Review[];
}
interface ProductImage {
  id: number;
...
export default function ProductDetailClient({
  ...
  const handleBuyNow = () => {
    ...
    // Redirect to the payment page with the selected product ID
    window.location.href = `/payment?ids=${product?.id || 0}`;
  };
  const handleSubmitReview = async () => {
    const token =
      typeof window !== "undefined" ? localStorage.getItem("token") : null;
  ...
  const fetchReviews = async () => {
    try {
      const res = await fetch(`${apiUrl}/api/review/${product?.id || 0}`);
      const data = await res.json();
      setReviews(data.data || []);
    } catch (error) {
  ...
  return (
    <>
      ...
      <AddToCartModal
        open={showModal}
        onClose={() => setShowModal(false)}
        productName={product.name || "Sản phẩm không có tên"}
        quantity={quantity}
        price={selectedVariant ? getEffectivePrice(selectedVariant) : 0}
        salePrice={selectedVariant?.sale_price}
        image={mainImg}
