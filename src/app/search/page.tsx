...
type Product = {
  ...
  slug?: string;
  img: { name: string }[];
  variant?: Record<string, unknown>; // Replaced `any` with `Record<string, unknown>`
};
...
const SearchPage = () => {
  ...
  const debouncedSetPriceRange = useCallback(debounce((value: number) => setPriceRange(value), 300), []);
  const debouncedSetSelectedSize = useCallback(debounce((value: string | null) => setSelectedSize(value), 300), []);
  ...
  useEffect(() => {
    ...
    fetchProducts(page);
  }, [keyword, selectedSize, priceRange, sortDirection, selectedCategoryId, page]);
